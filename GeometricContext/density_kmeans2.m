function maps = density_kmeans2(segDensity, spData, nSegments)
    ...
    for n = 1:length(nSegments)
        ...
        while updating && i<max_iters
            for r=1:num_regions
                %compute distance from in-cluster pixels to out-of-cluster
                %pixels
                in_r = maps(:,n)==r;
                dists = zeros(nSp, sum(in_r), nFeatures);
                for i=1:nSp
                    dists(i,:,:) = abs(features(i,:) - features(in_r,:)); 
                end
                ...
                for f=1:nFeatures
                    density_dists = density_dists+nSp - sum(getKdeLikelihood(...
                        segDensity(f).log_ratio, segDensity(f).x, dists(:,:,f)), 1);
                    getKdeLikelihood(...
