...
for (i=1:nAngles) 
    orientationImages(:, :, i) = abs(conv2(grayim,doogFilters(:, :, i),'same')-double(grayim));
end
...
for i = 1:length(imsegs)
    ...
    features(:, cf + (1:3)) = rgb;
    %ST: create colormap normalized to [0, 1], otherwise rb2hsv crashes
    rgbMax = max(rgb, [], 1);
    rgbColormap = rgb./max(1,rgbMax); 
    
    features(:, cf + (4:6)) = rgb2hsv(rgbColormap);
